...
- id: mahyoub
  ...
  issued:
    year: 2019
- id: dataset_uci
  title: Drug Consumption (Quatified) at UCI Machine Learning Repository
...
The regularized discriminant analysis was proposed by [@rda] as a trade-off between the linear discriminant analysis (LDA) in the Fisher inpretation [@lda], where a common (pooled) covariance matrix estimate $\hat{\Sigma}$ is used, and the quadratic discriminant analysis (QDA) [@qda], where an individual covariance matrix estimate $\hat{\Sigma}_k$ per class $k$ is used. This allows for quadratic classification boundaries. The trade-off is controlled by two mixing factors $\lambda$ and $\gamma$:
